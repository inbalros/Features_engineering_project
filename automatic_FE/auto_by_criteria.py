    ...
    for oper in operators_binary_direction_important:
        for f1 in features_binary:
            for f2 in features_binary:
                new_features_names.append(prepare_new_feature(True,oper,f1,f2))
    for oper in operators_binary_direction_NOT_important:
        for f1 in features_binary:
            for f2 in features_binary:
                new_features_names.append(prepare_new_feature(False,oper,f1,f2))
    for oper in operators_unary:
        for f1 in features_unary:
            new_features_names.append(prepare_new_feature(False,oper,f1,None))
...
def choose_best_feature(base_acu_test,base_criterion,criteria_Function,depth=None,number_of_kFolds=5,number_of_trees_per_fold=5):
    best_critrion ={}
    global data
    global X_names
    X_names.append('new_f')
    for name in new_features_names:
        if name not in added_f_names:
            #data['new_f']= dic_new_features[name]
            data.loc[:,'new_f']= dic_new_features[name]
            new_pred,clf = predict_kfold(data, X_names, y_names,criteria_Function,'new_f',number_of_trees_per_fold,number_of_kFolds,depth)
            new_acu_test = new_pred[0]
            new_criterion = new_pred[1]
            if new_acu_test > (base_acu_test-0.05) and new_criterion < base_criterion:
    ...
    data = data.rename(columns={'new_f': best_f[0]})
    X_names = [best_f[0] if x == 'new_f' else x for x in X_names]
    return best_f


def auto_F_E(number_of_kFolds,number_of_trees_per_fold,data_cur,X_names_cur,y_names_cur,features_unary,features_binary,depth,result_path,criteria_Function):
    global new_features_names
    global data
    global X_names
    global y_names
    ...
    for num in range (5):
        ...
        new_acc = max(base_acu_test,acu_test)
        new_f_name,(acu_test,criterion_cur,clf) = choose_best_feature(new_acc,criterion_cur,criteria_Function,depth,number_of_kFolds,number_of_trees_per_fold)
        if new_f_name == None:
            break
        rounds=rounds+1
